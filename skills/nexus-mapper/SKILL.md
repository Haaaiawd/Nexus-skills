...
### 地图存在时
如果仓库中存在 .nexus-map/INDEX.md，开始任务前必须先阅读它恢复全局上下文，再按其中的「操作指南」路由块决定下一步动作。

### 地图不存在时
如果 .nexus-map/ 不存在，且当前任务涉及以下任一情况：跨模块调用、接口修改、架构理解、多文件重构。
则应先向用户提议：「是否先运行 nexus-mapper 建立项目知识库？预计执行时间约 N 分钟。」
如果用户希望立即开始，至少先运行 `query_graph.py --summary` 建立最低限度的结构感知，不要对陌生大型仓库盲改核心接口。

### 结构查询（随时可用）
任何时候需要判断局部结构、依赖关系、影响半径或边界归属，优先使用 query_graph.py 基于 ast_nodes.json 做验证，不要凭目录名猜测。

### 知识库同步
当一次任务改变了项目的结构认知时（系统边界、入口、依赖关系、测试面、路线图），在任务完成后评估是否需要重新运行 nexus-mapper 更新 .nexus-map。纯局部实现细节默认不更新。
```
